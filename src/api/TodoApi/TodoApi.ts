...
import {
  ICreateTaskResponse, IDeleteTaskPayload, IPartialTodoResponseConverted, ITodoResponse, ITodoResponseConverted,
} from './types';
...
const TodoApi = () => {
  ...
  const getTodos = (authToken: string, data?: IToDoPayload) => api.sendQuery<ITodoResponse[], ITodoResponseConverted[]>({
  ...
  const patchToDo = (body: IPartialTodoResponseConverted, authToken: string) => api.sendQuery<object, object>({
  ...
  const createTodo = (body: IPartialTodoResponseConverted, authToken: string) => api.sendQuery<ICreateTaskResponse, ITodoResponseConverted>({
