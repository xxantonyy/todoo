...
const useFilters = () => {
  ...
  const handleChangeFilters = (value: unknown, key: keyof IToDoPayload) => {
    switch (key) {
    case 'priority':
  ...
  const handleSendFilters = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const handleRefreshFilters = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
